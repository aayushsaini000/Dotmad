...
class DotMadSpider(scrapy.Spider):
    name = "dotmad"
    handle_httpstatus_all = True
    
    start_urls = [
            'https://www.dotmed.com'
        ]
    ...
    def parse(self,response):
        validating = response.css("#topNavMenu > ul:nth-child(1) > li:nth-child(2) > ul a::attr(href)").extract()
        if validating:
            for validate in validating:
                validateurl = urljoin(response.url, validate)
                yield scrapy.Request(url=validateurl,callback=self.TargetUrl, meta={'handle_httpstatus_list': [301]},dont_filter = True)


    def TargetUrl(self,response):
        if response.status in [301, 302]:
            redirecturl = "https://www.dotmed.com"+response.css("a::attr(href)").extract()[0]
        else:
            redirecturl = response.url
        yield scrapy.Request(url = redirecturl, callback=self.FetchCatg,dont_filter = True) 
